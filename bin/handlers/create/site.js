...
import { createSiteConfig, createSitePackage } from "../../utils/config.js";
import { logger } from "../../utils/logger.js";

const __dirname = path.dirname(fileURLToPath(import.meta.url));

export async function createSiteHandler(name, options = {}) {
  ...
  // Resolve the target path - if user provided a relative path, it will be resolved relative to CWD
  const targetPath = path.resolve(process.cwd(), userPath);

  // logger.debug(`Resolved target path: ${targetPath} (from: ${userPath})`);
  ...
  try {
    ...
    applyTemplate(TEMPLATE_TYPES.SITE, sitePath, options);

    // await createSitePackage(sitePath, {
    //   name: siteName,
    //   description,
  ...
  try {
    ...
    for (const entry of entries) {
      if (entry.isDirectory() && entry.name.startsWith("site")) {
        // Check if it's a valid site directory (has site.yml)
        try {
          const configPath = path.join(basePath, entry.name, "site.yml");
